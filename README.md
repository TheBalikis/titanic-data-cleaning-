...
It contains demographic and travel information for passengers aboard the RMS Titanic and is commonly used for data quality and preprocessing practice.
 The dataset contains 419 rows of passenger data and 12 (original) columns present (13 after cleaning, with the addition of the Cabin Indicator column). The row count remained unchanged throughout the cleaning process to avoid data loss.
...
Only 2 of 891 fare records (0.2%) contained missing values- a negligible proportion that posed minimal risk of introducing bias. Median imputation was applied right than mean, as fare values were right-skewed due to extreme outliers among first-class passengers.
...
Confirmed all categorical columns (Sex, Embarked, Pclass, and Tittle) had consistent standardized label.
...
•	High numbers of missing values in cabin
Created a binary indicator instead of guessing value
...
The creation of the  has _ cabin indicator enables analysts to test whether passengers with recorded cabin assignments experienced different survival outcomes compared to those without assigned cabins. From a business and analytical perspective, this variable:
•	2 missing Fare values (0.2%) were resolved through median imputation, appropriate given right-skewed fare distribution
...
•	Improves model interpretability when used as a categorical feature
